...
"""
修复重复词牌：
- ID 36(望海潮重复) → 沁园春(114字)
- ID 50(谒金门重复) → 水龙吟(102字)

格律来源：52shici.com 钦定词谱 + 龙榆生《唐宋词格律》
"""
...
QINYUANCHUN_SENTENCES = [
    ...
    {"chars": 4, "rhyme": False, "tone": "仄仄平平仄"},
    {"chars": 4, "rhyme": False, "tone": "平平仄仄"},
    ...
    {"chars": 4, "rhyme": False, "tone": "平平仄仄"},
    {"chars": 7, "rhyme": True,  "rhyme_type": "平", "tone": "仄仄平平平仄平"},
    {"chars": 3, "rhyme": False, "tone": "仄平平仄"},
    {"chars": 5, "rhyme": True,  "rhyme_type": "平", "tone": "仄仄平平"},
]

qinyuanchun_total = sum(s["chars"] for s in QINYUANCHUN_SENTENCES)
print(f"沁园春总字数: {qinyuanchun_total}")

# ============================================================
# 水龙吟 102字 - ID 50
# ============================================================
SHUILONGYIN_SENTENCES = [
    # 上片
    {"chars": 6, "rhyme": True,  "rhyme_type": "仄", "tone": "中平中仄平平仄"},
...
shuilongyin_total = sum(s["chars"] for s in SHUILONGYIN_SENTENCES)
print(f"水龙吟总字数: {shuilongyin_total}")
...
qinyuanchun_entry = {
    ...
    "patterns": [
        {
            "id": "沁园春_zhengti",
            "description": "正体，苏轼体，114字",
            "name": "正体",
            "total_chars": 114,
            "rhyme_scheme": "",
            "sentences": QINYUANCHUN_SENTENCES
        },
        {
            "id": "沁园春_bianti_liuguo",
            "description": "变体，刘过体，句法疏放",
            "name": "刘过体",
            "total_chars": 114,
            "rhyme_scheme": "",
            "sentences": [
        ...
        {
            "id": "沁园春_bianti_qinguandjian",
            "description": "变体二，秦观减字体，又名洞庭春色",
            "name": "秦观减字体",
            "total_chars": 112,
            "rhyme_scheme": "",
            "sentences": [
    ...
    "patterns": [
        {
            ...
            "total_chars": 102,
            "rhyme_scheme": "",
            "sentences": SHUILONGYIN_SENTENCES
        },
        {
            ...
            "rhyme_scheme": "",
            "sentences": [
                {"chars": 6, "rhyme": True,  "rhyme_type": "仄", "tone": "中平中仄平平"},
                {"chars": 7, "rhyme": True,  "rhyme_type": "仄", "tone": "中仄中平中仄仄平平"},
                {"chars": 4, "rhyme": False, "tone": "中平中仄"},
                {"chars": 4, "rhyme": False, "tone": "中平中仄"},
                {"chars": 4, "rhyme": True,  "rhyme_type": "仄", "tone": "中平中仄"},
                {"chars": 6, "rhyme": False, "tone": "中仄中平中仄"},
...
for c in cp.CIPAI_DATABASE:
    ...
    if cid == 36:
        ...
        print(f'ID 36 → 沁园春 (正体+2变体)')
    elif cid == 50:
        new_db.append(shuilongyin_entry)
        print(f'ID 50 → 水龙吟 (正体+2变体)')
    else:
        new_db.append(c)

# 验证沁园春
qc = [c for c in new_db if c['id'] == 36][0]
print(f'沁园春 patterns: {[p["name"] for p in qc["patterns"]]}')
for p in qc['patterns']:
    s_chars = sum(s['chars'] for s in p['sentences'])
    print(f'  {p["name"]}: {len(p["sentences"])}句, 总字数={s_chars}')

# 验证水龙吟
sl = [c for c in new_db if c['id'] == 50][0]
print(f'水龙吟 patterns: {[p["name"] for p in sl["patterns"]]}')
